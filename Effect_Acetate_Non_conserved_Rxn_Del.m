% Program to check for Aceate production upon deletion of Non conserved
% reactions in each of the 30 different Environments
clc
clear
        ...
        for Bif_enz=1:length(Conserved_Rxns)
            ...
            if(Rxn_ID(Org,C_source)~=0)
                ...
                if (Non_conserved_soln{Org,C_source}(Bif_enz,1).f>1e-02)
                    ...
                    [minFlux_19_rxn_Del_Acetate{Org,C_source}(Bif_enz),maxFlux_19_rxn_Del_Acetate{Org,C_source}(Bif_enz)] =fluxVariability(New_model,100,'max',New_model.rxns(SCF_Lac_ID{Org,1}));
