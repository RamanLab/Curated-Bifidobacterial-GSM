% Program to check for Lactate production upon deletion of Conserved
    ...
    for C_source=1:length(Saccharides)
        ...
        for Bif_enz=1:length(Conserved_Rxns)
            ...
            New_model.lb(Bifid_Major_Enzy(Org,Bif_enz))=0;
              ...
              Enz_Del_Gr{Org,C_source}(Bif_enz,1)=soln{Org,C_source}(Bif_enz,1).f>1e-03;
  
          
            if (soln{Org,C_source}(Bif_enz,1).f>1e-02)
                
                [minFlux_7_rxn_Del_Lac{Org,C_source}(Bif_enz),maxFlux_7_rxn_Del_Lac{Org,C_source}(Bif_enz)] =fluxVariability(New_model,100,'max',New_model.rxns(SCF_Lac_ID{Org,1}));
